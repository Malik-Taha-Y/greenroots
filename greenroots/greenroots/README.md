...
**What it does:** both flows send the user's structured answers to a Gemini model, which
returns strict JSON (parsed and rendered directly into the result cards — no free-text
guessing on the frontend).

**Where it lives:** `app/api/recommend/route.ts` calls the Gemini API server-side (via the
`@google/genai` npm package), using the system prompts defined in `lib/prompts.ts`.
...
- **AI:** Google Gemini API (`@google/genai` npm package), model `gemini-2.5-flash` by
  default (configurable via `GEMINI_MODEL` env var). Gemini has a genuine free tier, which is
...
**Requirements:** Node.js 18.18+ and a (free) Gemini API key from
...
1. Push this repo to a **public** GitHub repository (see step-by-step below if you're new to
   this).
2. Go to https://vercel.com, sign in with GitHub, click **Add New → Project**, and import the
   `greenroots` repo.
3. Vercel auto-detects Next.js — leave the build settings as default.
4. Before deploying, open **Environment Variables** and add:
   - `GEMINI_API_KEY` = your real key from https://aistudio.google.com/api-keys
...
7. Open the live URL in an incognito window and run through both flows once — this is also
   where you'll grab your 3 screenshots (homepage, one wizard step, a results page).
